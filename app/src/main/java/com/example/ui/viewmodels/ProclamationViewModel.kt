...
class ProclamationViewModel(
    ...
    fun selectTopic(topic: ProclamationTopicEntity) {
        _selectedTopic.value = topic
        _topicText.value = topic.topic
        _targetCount.value = topic.targetCount
        // Keep current session count or start fresh
    }
    ...
    fun incrementCounter(amount: Int = 1) {
        ...
        if (!_isTimerRunning.value && _counter.value > 0) {
            startTimer()
        }
    }
    ...
    fun setCounterValue(value: Int) {
        _counter.value = value.coerceAtLeast(0)
    }
    ...
    fun saveSession(onSuccess: () -> Unit = {}) {
        viewModelScope.launch {
            ...
            accountabilityRepository.logNotification(
                ...
                message = "Proclaimed '$finalTopic' $count times (${duration / 60}m). Victory in Jesus!"
            )
